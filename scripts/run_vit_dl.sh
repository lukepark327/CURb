...
DATASET="${DATASET:-domainnet}"
DATA_ROOT="${DATA_ROOT:-./data}"
TRAIN_DIR="${TRAIN_DIR:-}"
VAL_DIR="${VAL_DIR:-}"
NUM_TASKS="${NUM_TASKS:-6}"
TOTAL_ROUND="${TOTAL_ROUND:-5}"
...
SAVE_ROOT="${SAVE_ROOT:-./cl_runs/vit_domain_compare}"
EXP_TAG="${EXP_TAG:-vit_dl_compare_$(date +%Y%m%d_%H%M%S)}"
...
log "Epochs=${EPOCHS} (Domain-IL default in vit_dl.py=15, overridden here)"
...
extra_orders_args=()
if [[ -n "${DOMAIN_ORDERS_JSON}" ]]; then
  extra_orders_args+=(--domain_orders_json "${DOMAIN_ORDERS_JSON}")
fi
...
if [[ ! -f "${splits_json}" ]]; then
  ...
  splits_cmd=(
    "${PYTHON_BIN}" -u experiments/vit/vit_dl.py
    --make_splits_only
    --dataset "${DATASET}"
    --data_root "${DATA_ROOT}"
  ...
  log "seed=${seed} building domain splits -> ${splits_json}"
...
if (( overall_failed )); then
  log "ViT Domain-IL launcher finished with failures. status=${STATUS_CSV}"
  exit 1
fi
log "ViT Domain-IL launcher completed successfully. status=${STATUS_CSV}"
